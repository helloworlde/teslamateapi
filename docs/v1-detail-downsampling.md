...
| 接口 | 明细数组 | 默认行为 |
| --- | --- | --- |
| `GET /api/v1/cars/{CarID}/drives/{DriveID}` | `data.drive.drive_details` | 为保证向后兼容，默认仍为 `sample=every_5s`。 |
...
| 值 | 含义 | 推荐客户端场景 |
| --- | --- | --- |
| `auto` | 原始点数不超过 `max_points` 时返回全量；超过时保留时间桶首尾、极值和状态变化点。 | 充电详情默认值；推荐图表、移动端和常规详情页使用。 |
| `full` | 返回所有原始明细行。 | 导出、调试，或明确需要原始数据的客户端。 |
| `every_5s` | 旧的固定时间桶采样，每 5 秒桶保留第一行。 | 既有行程详情客户端；兼容模式。 |
| `every_30s` | 旧的固定时间桶采样，每 30 秒桶保留第一行。 | 非常轻量的轨迹/曲线预览。 |

行程详情默认 `every_5s`；充电详情默认 `auto`。
...
- 最小接受值：`100`
- 最大接受值：`3000`
- 这是目标值，不是硬上限。状态变化点和关键极值会强制保留，因此特殊会话的返回数量可能超过 `max_points`。
