...
func TeslaMateAPICarsChargesCurrentV1(c *gin.Context) {
	...
	row := apicommon.DB.QueryRow(query, CarID)
	...
	switch err {
	case sql.ErrNoRows:
		apicommon.HandleErrorResponse(c, "TeslaMateAPICarsChargesCurrentV1", "No current charge found.", "No rows were returned")
		return
	case nil:
		// nothing wrong.. continuing
		break
	default:
		apicommon.HandleErrorResponse(c, "TeslaMateAPICarsChargesCurrentV1", CarsChargesCurrentError1, err.Error())
		return
	}
	...
	if UnitsLength == "mi" {
		charge.RatedRange.StartRange = conv.KmToMi(charge.RatedRange.StartRange)
		charge.RatedRange.CurrentRange = conv.KmToMi(charge.RatedRange.CurrentRange)
		charge.RatedRange.AddedRange = conv.KmToMi(charge.RatedRange.AddedRange)
	...
	detailsQuery := `
		...
		ORDER BY id DESC;`
	rows, err := apicommon.DB.Query(detailsQuery, charge.ChargeID)

	// Checking for errors in query
	if err != nil {
		apicommon.HandleErrorResponse(c, "TeslaMateAPICarsChargesCurrentV1", CarsChargesCurrentError2, err.Error())
		return
	}
	...
	if len(ChargeDetailsData) > 0 {
		...
		if err != nil {
			apicommon.HandleErrorResponse(c, "TeslaMateAPICarsChargesCurrentV1", CarsChargesCurrentError2, "Error parsing charge detail date")
			return
		}
		...
		if timeElapsed.Minutes() > maxChargeInactivityThresholdMinutes {
			apicommon.HandleErrorResponse(c, "TeslaMateAPICarsChargesCurrentV1", CarsChargesCurrentError3, "No active charging in progress. There are incomplete charges but last update was more than 15 minutes ago.")
			return
		}
	}
	...
	// Return jsonData
	apicommon.HandleSuccessResponse(c, "TeslaMateAPICarsChargesCurrentV1", jsonData)
}
