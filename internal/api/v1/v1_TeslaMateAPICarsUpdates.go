...
func TeslaMateAPICarsUpdatesV1(c *gin.Context) {
	...
	query := `
		...
		LIMIT $2 OFFSET $3;`
	rows, err := apicommon.DB.Query(query, CarID, ResultShow, ResultPage)

	// checking for errors in query
	if err != nil {
		apicommon.HandleErrorResponse(c, "TeslaMateAPICarsUpdatesV1", CarsUpdatesError1, err.Error())
		return
	...
	for rows.Next() {
		...
		update.StartDate = apicommon.GetTimeInTimeZone(update.StartDate)
		update.EndDate = apicommon.GetTimeInTimeZone(update.EndDate)
	...
	// return jsonData
	apicommon.HandleSuccessResponse(c, "TeslaMateAPICarsUpdatesV1", jsonData)
}
