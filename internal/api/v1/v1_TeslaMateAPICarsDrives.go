...
func TeslaMateAPICarsDrivesV1(c *gin.Context) {
	...
	if err != nil {
		apicommon.HandleErrorResponse(c, "TeslaMateAPICarsDrivesV1", CarsDrivesError2, err.Error())
		return
	}
	...
	minDistance := 0.0
	if minDistanceParam != "" {
		minDistance = apicommon.ConvertStringToFloat(minDistanceParam)
		if minDistance < 0 {
			minDistance = 0
		}
	}
	...
	maxDistance := 0.0
	if maxDistanceParam != "" {
		maxDistance = apicommon.ConvertStringToFloat(maxDistanceParam)
		if maxDistance < 0 {
			maxDistance = 0
		}
	}
	...
	if minDistance > 0 || maxDistance > 0 {
		...
		err = apicommon.DB.QueryRow("SELECT unit_of_length FROM settings LIMIT 1").Scan(&unitsLength)
		if err != nil {
			apicommon.HandleErrorResponse(
				c,
		...
		if unitsLength == "mi" {
			if minDistance > 0 {
				minDistance = conv.MiToKm(minDistance)
			}
			if maxDistance > 0 {
				maxDistance = conv.MiToKm(maxDistance)
			}
		}
	...
	if err != nil {
		apicommon.HandleErrorResponse(c, "TeslaMateAPICarsDrivesV1", CarsDrivesError1, err.Error())
		return
	}
	...
	// return jsonData
	apicommon.HandleSuccessResponse(c, "TeslaMateAPICarsDrivesV1", jsonData)
}
