...
func (h V2Handlers) CapacityByMileage(c *gin.Context) {
	...
	if h.capacityByMileageBuilder == nil {
		apicommon.V2Error(c, http.StatusInternalServerError, "INTERNAL_ERROR", "V2 capacity-by-mileage service is not configured.", nil)
		return
	}
	...
	if err != nil {
		...
		default:
			apicommon.V2Error(c, http.StatusInternalServerError, "INTERNAL_ERROR", "Unable to build V2 capacity by mileage.", err.Error())
		}
		return
	}
