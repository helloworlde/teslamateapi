...
func (r PostgresV2ChargingRepository) Summary(ctx context.Context, carID int64, start timeBound, end timeBound) (V2ChargingSummary, V2ChargingStats, error) {
	...
	var medianPowerDC sql.NullFloat64
	var maxPowerAC sql.NullFloat64
	var maxPowerDC sql.NullFloat64
	...
	err := r.db.QueryRowContext(ctx, `
		SELECT
			...
			PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY max_charge.charger_power) FILTER (WHERE max_charge.charger_power >= 20) AS median_power_dc,
			MAX(max_charge.charger_power) FILTER (WHERE COALESCE(max_charge.charger_power, 0) > 0 AND max_charge.charger_power < 20) AS max_power_ac,
			MAX(max_charge.charger_power) FILTER (WHERE max_charge.charger_power >= 20) AS max_power_dc,
	...
	if avgPowerDC.Valid {
		summary.AvgPowerDC = &avgPowerDC.Float64
	}
	if medianPowerAC.Valid {
		summary.MedianPowerAC = &medianPowerAC.Float64
	}
	if medianPowerDC.Valid {
		summary.MedianPowerDC = &medianPowerDC.Float64
	}
	if maxPowerAC.Valid {
		summary.MaxPowerAC = &maxPowerAC.Float64
	}
	if maxPowerDC.Valid {
		summary.MaxPowerDC = &maxPowerDC.Float64
	}
...
func (r PostgresV2ChargingRepository) Timeseries(ctx context.Context, carID int64, timeRange V2TimeRange, groupBy string) ([]V2ChargingTimeseriesItem, V2ChargingStats, error) {
	...
	for rows.Next() {
		...
		if avgPowerDC.Valid {
			item.AvgPowerDC = &avgPowerDC.Float64
		}
