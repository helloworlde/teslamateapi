...
func (h V2Handlers) ChargingCurve(c *gin.Context) {
	...
	if h.chargingCurveBuilder == nil {
		apicommon.V2Error(c, http.StatusInternalServerError, "INTERNAL_ERROR", "V2 charging curve service is not configured.", nil)
		return
	}
	minSessions := 5
	...
	if err != nil {
		...
		default:
			apicommon.V2Error(c, http.StatusInternalServerError, "INTERNAL_ERROR", "Unable to build V2 charging curve.", err.Error())
		}
		return
	}
