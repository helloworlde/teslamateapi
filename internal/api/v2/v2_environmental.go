...
func (h V2Handlers) Environmental(c *gin.Context) {
	...
	if h.environmentalBuilder == nil {
		apicommon.V2Error(c, http.StatusInternalServerError, "INTERNAL_ERROR", "V2 environmental service is not configured.", nil)
		return
	}
	...
	if err != nil {
		...
		default:
			apicommon.V2Error(c, http.StatusInternalServerError, "INTERNAL_ERROR", "Unable to build V2 environmental analytics.", err.Error())
		}
		return
	}
