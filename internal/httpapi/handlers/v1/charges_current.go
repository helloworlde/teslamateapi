...
func (h *Handler) ChargesCurrent(c *gin.Context) {
	...
	query := `
		SELECT
			...
			EXTRACT(EPOCH FROM (COALESCE(end_date, (CURRENT_TIMESTAMP AT TIME ZONE 'UTC')) - start_date))/60 AS duration_min,
			TO_CHAR((EXTRACT(EPOCH FROM (COALESCE(end_date, (CURRENT_TIMESTAMP AT TIME ZONE 'UTC')) - start_date))/60 * INTERVAL '1 minute'), 'HH24:MI') as duration_str,
	...
	switch err {
	case sql.ErrNoRows:
		respond.HandleError(c, "TeslaMateAPICarsChargesCurrentV1", "No current charge found.", "No rows were returned")
		return
	case nil:
		// nothing wrong.. continuing
		break
	default:
		respond.HandleError(c, "TeslaMateAPICarsChargesCurrentV1", CarsChargesCurrentError1, err.Error())
		return
	}
	...
	if UnitsLength == "mi" {
		charge.RatedRange.StartRange = convert.KilometersToMiles(charge.RatedRange.StartRange)
		charge.RatedRange.CurrentRange = convert.KilometersToMiles(charge.RatedRange.CurrentRange)
		charge.RatedRange.AddedRange = convert.KilometersToMiles(charge.RatedRange.AddedRange)
	...
	detailsQuery := `
		...
		ORDER BY id DESC;`
	rows, err := h.db.QueryContext(c.Request.Context(), detailsQuery, charge.ChargeID)

	// Checking for errors in query
	if err != nil {
		respond.HandleError(c, "TeslaMateAPICarsChargesCurrentV1", CarsChargesCurrentError2, err.Error())
		return
	}
	...
	if len(ChargeDetailsData) > 0 {
		...
		if err != nil {
			respond.HandleError(c, "TeslaMateAPICarsChargesCurrentV1", CarsChargesCurrentError2, "Error parsing charge detail date")
			return
		}
		...
		if timeElapsed.Minutes() > maxChargeInactivityThresholdMinutes {
			respond.HandleError(c, "TeslaMateAPICarsChargesCurrentV1", CarsChargesCurrentError3, "No active charging in progress. There are incomplete charges but last update was more than 15 minutes ago.")
			return
		}
	}
	...
	// Return jsonData
	respond.HandleSuccess(c, "TeslaMateAPICarsChargesCurrentV1", jsonData)
}
