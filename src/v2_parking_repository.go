...
func (r PostgresV2ParkingRepository) Summary(ctx context.Context, carID int64, timeRange V2TimeRange) (V2ParkingAnalyticsSummary, V2ParkingStats, error) {
	var summary V2ParkingAnalyticsSummary
...
func (r PostgresV2ParkingRepository) StateBreakdown(ctx context.Context, carID int64, timeRange V2TimeRange) (V2ParkingStatesResponse, V2ParkingStats, error) {
	rows, err := r.db.QueryContext(ctx, `
		SELECT
			...
			COALESCE(SUM(EXTRACT(EPOCH FROM (LEAST(COALESCE(end_date, $3::timestamptz), $3::timestamptz) - GREATEST(start_date, $2::timestamptz))) / 60), 0) AS duration_min
		FROM states
		WHERE car_id = $1
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.State, &item.TransitionCount, &item.DurationMin); err != nil {
			return response, stats, err
		}
		response.TotalDurationMin += item.DurationMin
	...
	for i := range response.Items {
		if response.TotalDurationMin > 0 {
			response.Items[i].Percent = response.Items[i].DurationMin / response.TotalDurationMin * 100
		}
	}
...
func (r PostgresV2ParkingRepository) Locations(ctx context.Context, carID int64, timeRange V2TimeRange) ([]V2ParkingLocationItem, V2ParkingStats, error) {
	rows, err := r.db.QueryContext(ctx, `
		...
		SELECT
			...
			COALESCE(SUM(EXTRACT(EPOCH FROM (LEAST(parking_end, $3::timestamptz) - GREATEST(parking_start, $2::timestamptz))) / 60), 0) AS parked_duration_min
		FROM sessions
		...
		GROUP BY 1, 2, 3
		ORDER BY parked_duration_min DESC`,
		carID, asTimeBound(timeRange.Start).Time, asTimeBound(timeRange.End).Time,
	)
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.LocationName, &geofenceID, &addressID, &item.ParkingSessionCount, &item.ParkedDurationMin); err != nil {
		...
		if item.ParkingSessionCount > 0 {
			item.AvgParkedDurationMin = float64Ptr(item.ParkedDurationMin / float64(item.ParkingSessionCount))
		}
...
func (r PostgresV2ParkingRepository) loadStateSummary(ctx context.Context, carID int64, timeRange V2TimeRange, summary *V2ParkingAnalyticsSummary, stats *V2ParkingStats) error {
	err := r.db.QueryRowContext(ctx, `
		...
		WHERE car_id = $1
			...
			AND COALESCE(end_date, $3::timestamptz) > $2::timestamptz`,
		carID, asTimeBound(timeRange.Start).Time, asTimeBound(timeRange.End).Time,
	).Scan(&stats.StateRows, &summary.ParkedDurationMin, &summary.AsleepDurationMin, &summary.OnlineDurationMin, &summary.OfflineDurationMin)
	if err != nil {
		return err
	}
...
func (r PostgresV2ParkingRepository) loadSessionSummary(ctx context.Context, carID int64, timeRange V2TimeRange, summary *V2ParkingAnalyticsSummary, stats *V2ParkingStats) error {
	err := r.db.QueryRowContext(ctx, `
		WITH events AS (
		...
		SELECT
			COUNT(*) AS parking_session_count,
			AVG(EXTRACT(EPOCH FROM (LEAST(parking_end, $3::timestamptz) - GREATEST(parking_start, $2::timestamptz))) / 60) AS avg_parked_duration_min
		FROM sessions
		WHERE parking_end IS NOT NULL
			AND parking_start < $3::timestamptz
			AND parking_end > $2::timestamptz
			AND parking_end > parking_start`,
		carID, asTimeBound(timeRange.Start).Time, asTimeBound(timeRange.End).Time,
	).Scan(&summary.ParkingSessionCount, &summary.AvgParkedDurationMin)
	if err != nil {
		return err
	}
...
func (r PostgresV2ParkingRepository) loadDrainSummary(ctx context.Context, carID int64, timeRange V2TimeRange, summary *V2ParkingAnalyticsSummary, stats *V2ParkingStats) error {
	...
	if startRange.Valid && endRange.Valid && startRange.Float64 > endRange.Float64 {
		...
		summary.VampireDrainRangeKM = &rangeLoss
		...
		if efficiency.Valid {
			summary.EstimatedVampireDrainKWh = float64Ptr(rangeLoss * efficiency.Float64)
		}
	}
	return nil
}
